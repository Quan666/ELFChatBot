...
from nonebot.adapters.cqhttp import Bot, unescape
...
async def handle_fake_msg(bot: Bot, event: Event, state: dict):
    ...
    msg_info = state["fake"]
    try:
        msg = await fake_forward(message=unescape(msg_info), group_id=event.group_id, bot=bot, user_id=event.user_id,
