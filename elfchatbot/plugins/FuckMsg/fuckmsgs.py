...
async def handle_first_receive(bot: Bot, event: Event, state: T_State, args: Message = CommandArg()):
    ...
    state["fake_messages"] = []
    if plain_text:
        state["fuck"] = plain_text  # 如果用户发送了参数则直接赋值


@fuck.got(
    "fuck",
    prompt="使用方法: (QQ号)|(@某人) [自定义昵称]%第一条消息[~第二条消息]\n% 后表示消息\n消息用 ~ 分割\n切勿用作违法！\n输入要伪造的消息(发送“结束伪造”结束)：",
)
...
async def fuck_forward(message, group_id, bot, user_id=None):
    ...
    for con in contents:
        ...
        for msg_tmp in msg_list:
            node = {
                "type": "node",
                "data": {"name": nickname, "uin": str(user), "content": msg_tmp},
            }
            msg.append(node)
        # msg.append({
        #     "type": "node",
